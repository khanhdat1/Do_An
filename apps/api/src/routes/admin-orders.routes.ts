 ...
 * "đã thanh toán" như VNPay). Chỉ tài khoản role ADMIN/STAFF vào được — xem `requireRole`.
 */
export const adminOrdersRouter = Router();

adminOrdersRouter.use(noStore, authenticate, requireAuth, requireRole("ADMIN", "STAFF"));
...
adminOrdersRouter.post("/:orderCode/confirm-payment", adminWriteLimiter, async (req, res, next) => {
  try {
